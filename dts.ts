...
export const dtsPare = (payload: string | null | undefined): DtsParseResult => {
    ...
    const rootNode = root.children.length === 1 && rootChild?.fullName === "/"
        ? rootChild
        : root;
...
const parseNodeHeader = (
    ...
    if (!match || !match.groups) {
        errors.push(`Malformed node declaration at statement ${lineNumber}: ${statement}`);
        return null;
    }
...
const normalizeNodeName = (
    ...
    if (!rawName) {
        errors.push(`Missing node name at statement ${lineNumber}: ${statement}`);
        return "<unknown>";
    }
    return rawName;
};
...
const parseProperty = (
    ...
    if (eqIndex === -1) {
        errors.push(`Malformed property at statement ${lineNumber}: ${statement}`);
        return null;
    }
    ...
    if (value === undefined) {
        errors.push(`Unable to parse value at statement ${lineNumber}: ${statement}`);
        return null;
    }
