...
const scriptPattern =
	/<script([^>]*?)\bsrc=["'][^"']*app\.(?:ts|js)["']([^>]*)>\s*<\/script>/i;

let finalHtml = html.replace(
	scriptPattern,
	(match, preAttrs = "", postAttrs = "") => {
		const attributeSource = `${preAttrs}${postAttrs}`;
		const withoutSrc = attributeSource.replace(
			/\s*src\s*=\s*["'][^"']*["']/i,
			"",
		);
...
if (finalHtml === html) {
	console.warn(
		"No <script> tag referencing app.ts or app.js found in index.html; script not inlined",
	);
}
