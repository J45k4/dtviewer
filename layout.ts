...
export const layoutTree = (
		...
		if (portalTargets) {
			portalTargets.forEach((target) => {
				...
				layoutNode.children.push(
					buildLayoutNode(target, depth + 1, node.path),
				);
			});
		}
	...
	depths.forEach((depth) => {
		...
		if (count > 1) {
			...
			const sinHalf = Math.max(
				Math.sin(Math.min(Math.PI / 2, theta / 2)),
				1e-6,
			);
	...
	const shiftSubtree = (node: LayoutNode, delta: number) => {
		...
		while (stack.length) {
			...
			current.radius =
				radiusByDepth.get(current.depth) ?? current.depth * layerGap;
			current.x =
				current.radius * Math.cos(current.angle) - NODE_WIDTH / 2;
	...
	nodesByDepth.forEach((nodesAtDepth, depth) => {
		...
		for (let index = 0; index < count - 1; index += 1) {
			originalGaps[index] =
				(unwrapped[index + 1] ?? 0) - (unwrapped[index] ?? 0);
		}
		...
		for (let index = 1; index < count; index += 1) {
			targetAngles[index] =
				(targetAngles[index - 1] ?? 0) +
				(gapSizes[index - 1] ?? baseGap);
		}
		const offset = sorted[0]!.angle - ((targetAngles[0] ?? 0) - Math.PI);
